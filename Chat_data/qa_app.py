...
st.set_page_config(page_title="Argaam",page_icon=':shark:')
...
def main():
    ...
    language = st.sidebar.radio(
        "Choose Language/اختر اللغة", ["English/إنجليزي", "Arabic/عربي"])
    ...
    if uploaded_files:
       ...
       # Display the question-answer pairs in the sidebar with smaller text
        ...
        if user_question:
            if language == "Arabic/عربي":
                answer = qa.run(user_question)
                st.write("Answer:", answer)


            elif language == "English/إنجليزي":
                #answer = qa.run(user_question)
                answer = qa.run(user_question)
                st.write("Answer:", answer)
